...
class IndexView(EventPermissionRequiredMixin, ChartContainingView, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not ctx['abd_data'] or not ctx['abt_data']:
            opqs = OrderPosition.objects.filter(order__event=self.request.event, item__admission=True).annotate(
            ...
            paid_by_day = {}
            for p in opqs.filter(payment_date__isnull=False).values('payment_date'):
                print(p)
